...
class WordsData(object):
    ...
    def _load_data(self, asl, fn, feature_list):
        ...
        dict = {}
        """  read the values from the `train_words.csv` one row at a time,
        walk through every frame ([startframe ... endframe]), look up
        (video,frame) row in the asl db, capture the specified "feature values"
        from there, and add it a dictionary, whose key is the word in current row
        of `train_words.csv`
        """
...
class SinglesData(object):
    ...
    """  read the values from the `test_words.csv` one row at a time,
        walk through every frame ([startframe ... endframe]), look up
        (video,frame) row in the asl db, capture the specified "feature values"
        from there, and add it a dictionary, whose key is the index of  current row
        of `test_words.csv`
    """
