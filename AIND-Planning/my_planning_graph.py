...
class PlanningGraph():
        ...
        for literal in self.all_actions:
            ...
            if actionNode.prenodes.issubset(self.s_levels[level]):
                ...
                for s_node in self.s_levels[level]:
                    if s_node in actionNode.prenodes:
                        # print("=====================")
                        # print("StateNode = {} {}".format(s_node.symbol, s_node.is_pos))
                        s_node.children.add(actionNode)
                        actionNode.parents.add(s_node)
        # no mutexes at the first level
    ...
    def add_literal_level(self, level):
        ...
        for actionNode in self.a_levels[level-1]:
            for sNode in actionNode.effnodes:
                self.s_levels[level].add(sNode)
                actionNode.children.add(sNode)
                sNode.parents.add(actionNode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for n1 in node_a1.effnodes:
            for n2 in node_a2.effnodes:
                if n1.symbol == n2.symbol and n1.is_pos != n2.is_pos:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for e2 in node_a2.effnodes:
            for p1 in node_a1.prenodes:
                if e2.symbol == p1.symbol and e2.is_pos != p1.is_pos:
                    return True
        return False                                              
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for n1 in node_a1.prenodes:
            for n2 in node_a2.prenodes:
                if (n1.symbol == n2.symbol and n1.is_pos != n2.is_pos):
                    return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        if node_s1 == node_s1 and node_s1.is_pos != node_s2.is_pos:
            return True
        return False
    ...
    def h_levelsum(self) -> int:
        """The sum of the level costs of the individual Goalss (admissible if goals independent)
